...
func _init_astar(tile_map: TileMap):
	astar = AStar2D.new()
	var TILE_ADJ = tile_map.cell_quadrant_size / 2 # Смещение к центру тайла
	TILE_ADJ = Vector2(TILE_ADJ, TILE_ADJ)
	...
	for i in range(cells.size()):
		...
		astar.add_point(i, tile_map.map_to_local(cell) + TILE_ADJ)
		...
		for neighbour in neighbours:
			if tile_map.get_cell_source_id(0, neighbour) == INVALID_CELL:
				continue
			astar.connect_points(i, astar.get_closest_point(tile_map.map_to_local(neighbour) + TILE_ADJ))


# Возвращает путь (массив точек) от точки до точки
func get_move_path(from: Vector2, to: Vector2) -> PackedVector2Array:
...
func get_points_in_radius(point: Vector2, radius: int) -> PackedVector2Array:
	...
	for i in range(radius):
		...
		for open_candidate_i in range(cells_to_open.size()):
			...
			if ground_tile_map.get_cellv(cell_to_open) == INVALID_CELL or \
